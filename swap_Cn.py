...
number = int(st.number_input('frame'))
st.write('frame is ', number)
uploaded_file = st.file_uploader("Choose a file")
if uploaded_file is not None:
    dataframe = pd.read_csv(uploaded_file, index_col='Atom Index')
    st.write(dataframe)

if st.button('统计完全不变原子信息'):
    progress_text = "Operation in progress. Please wait."
    my_bar = st.progress(0, text=progress_text)
    num = len(dataframe)
    df = pd.DataFrame(columns=['frame', 'unchanged_number', 'change_numer'])
    for i in range(0, number):
        count = 0
        for j in range(num):
            first_num = dataframe.iloc[j, 0]
            now_num = dataframe.iloc[j, i]
            if now_num == first_num:
                count += 1
        change_num = num - count
        df.loc[i] = [i + 1, count, change_num]
        my_bar.progress((i+1) / number, text=progress_text)
    time.sleep(1)
    coordination_csv = convert_df(df)
    st.download_button(
        label="Download coordination as CSV",
        data=coordination_csv,
        file_name=f'unchanged_number_{cen_atom}.csv',
        mime='text/csv',
    )
else:
    st.write('Goodbye')